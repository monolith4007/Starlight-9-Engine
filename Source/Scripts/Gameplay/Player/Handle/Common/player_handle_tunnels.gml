...
if (!tunnel_lock)
{
    if (player_collision_check(COL_MAIN_OBJECT, MASK_MAIN, x, y, obj_sensor_lock_tunnel_right) && x_speed >= 0)
    {
        tunnel_lock = true;
        if (state != STATE_ROLL)
        {
    ...
    else if (player_collision_check(COL_MAIN_OBJECT, MASK_MAIN, x, y, obj_sensor_lock_tunnel_left) && x_speed <= 0)
    {
        tunnel_lock = true;
        if (state != STATE_ROLL)
        {
